...
export default async function handler(req: any, res: any) {
  try {
    ...
    // Extract the original path from various sources
    let apiPath = '/api';
    
    // Try to get path from Vercel's internal routing
    const originalUrl = req.url || req.originalUrl || '/';
    
    // Check if we have the path in the request
    if (originalUrl.startsWith('/api')) {
      ...
      apiPath = pathValue ? `/api/${pathValue}` : '/api';
    }

    // Update request properties for Express
    req.url = apiPath;
    req.originalUrl = apiPath;
    req.path = apiPath.split('?')[0]; // Remove query string from path
